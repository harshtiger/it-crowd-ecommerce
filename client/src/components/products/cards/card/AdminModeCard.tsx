...
import { deleteProduct, setInactiveProduct } from "../../../../redux/actions/admin";
...
const AdminModeCard = ({ name, image, price, id, orders, page, isActive, eliminateFilters }: props) => {
  ...
  function deleteHandler(e: React.MouseEvent<HTMLButtonElement>): void {
    ...
    swal({
      title: "Are you sure?",
      text: "This product is now going to be deleted from DB!",
      icon: "warning",
      dangerMode: true,
      buttons: {
        cancel: true,
  ...
  function setInactive(e: React.MouseEvent<HTMLButtonElement>): void {
    e.preventDefault();
    swal({
      title: "Are you sure?",
      ...
      if (value) {
        ...
        dispatch(setInactiveProduct(stringId, data, userInStorage.token));
        setTimeout(() => {
          dispatch(getProducts())
        }, 200)
        page(1)
